...
import { FormattedDateTime, JsonViewer, PageHeader, StatusBadge } from '@caddy-manager/ui';
...
export default function SiteOverview() {
  ...
  if (siteQuery.isLoading) return <div className="alert alert-info">Loading site overview...</div>;
  if (siteQuery.isError || !siteQuery.data) {
    return <div className="alert alert-danger">Unable to load this site overview.</div>;
  }
      ...
      <PageHeader
        ...
        actions={
          <div className="d-flex gap-2">
            <button type="button" className="btn btn-outline-secondary" onClick={() => navigate('/sites')}>
              <i className="bi bi-arrow-left me-1" /> Sites
            </button>
            {managedByManager && (
              <button type="button" className="btn btn-primary" onClick={() => navigate(`/sites/${site.id}/edit`)}>
          ...
          <div className="card h-100">
            <div className="card-body">
              ...
              <dl className="row mb-0">
                ...
                <dd className="col-sm-7">{site.statusDetail || 'No additional details'}</dd>
                <dt className="col-sm-5 text-muted">Last checked</dt>
                <dd className="col-sm-7"><FormattedDateTime value={site.lastCheckedAt} fallback="Not checked" /></dd>
                <dt className="col-sm-5 text-muted">Latency</dt>
                <dd className="col-sm-7">{site.healthLatencyMs !== undefined ? `${site.healthLatencyMs} ms` : 'Not measured'}</dd>
          ...
          <div className="card h-100">
            <div className="card-body">
              ...
              <dl className="row mb-0">
                ...
                <dd className="col-sm-8">{managedByManager ? 'Caddy Manager' : 'Caddyfile / external configuration'}</dd>
                <dt className="col-sm-4 text-muted">Handler</dt>
                <dd className="col-sm-8"><code>{handler}</code></dd>
                <dt className="col-sm-4 text-muted">Route ID</dt>
                <dd className="col-sm-8"><code>{site.routeId || 'No route ID'}</code></dd>
                <dt className="col-sm-4 text-muted">Server</dt>
                <dd className="col-sm-8">{server ? `${server.name} (${server.hostname})` : site.serverId}</dd>
                <dt className="col-sm-4 text-muted">TLS</dt>
                <dd className="col-sm-8">{site.tlsEnabled ? 'Enabled' : 'Disabled'}</dd>
              </dl>
            </div>
          </div>
        </div>
        ...
        <div className="col-12">
          <div className="card">
            ...
            <div className="card-body p-0">
              {site.routeConfig ? <JsonViewer data={site.routeConfig} title="Caddy route JSON" /> : <div className="p-4 text-muted">No route configuration is stored for this site.</div>}
            </div>
          </div>
        </div>
      </div>
